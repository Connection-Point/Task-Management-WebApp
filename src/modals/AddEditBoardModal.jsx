      ...
      <div className='scrollbar-hide overflow-y-scroll max-h-[95vh]  bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold
       shadow-md shadow-[#364e7e1a] max-w-md mx-auto my-auto w-full px-8  py-8 rounded-xl'>
        ...
        <div className='mt-8 flex flex-col space-y-3'>
          ...
          outline-none focus:outline-[#635fc7] outline-1 ring-0' placeholder='e.g Infrastructure Design' 
          value={name}
          onChange={(e) => {
          ...
          </label>
          {
            newColumns.map((column,index) => (
              <div key={index} className='flex items-center w-full'>
                <input type='text' className='bg-transparent flex-grow px-4 py-2 rounded-md text-sm border border-gray-600 outline-none focus:outline-[#735fc7]'
                onChange={(e) => {
                ...
                value = {column.name}
                />
                <img src={crossIcon} className='cursor-pointer m-4' 
                onClick={() => {
                  onDelete(column.id)
                }}/>
              </div>
        ...
        <div>
          <button className='w-full items-center hover:opacity-75 dark:text-[#635fc7] dark:bg-white text-white bg-[#635fc7] py-2 mt-2 rounded-full'
          onClick={() => {
           setNewColumns((state) =>[
            ...state,
            {name : '',task : [], id:uuidv4()},
